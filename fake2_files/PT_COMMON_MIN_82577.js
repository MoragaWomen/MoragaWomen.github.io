...
function ShowMoreFacetValues(multiSelect, facetGbId, facetGridId, moreId, lessId) {
 ...
 elemFacetValuesGrid.style.width = nPrntWidth + "px"; elemFacetValuesGrid.style.height = nGridHeight + "px";}
...
function ShowLessFacetValues(multiSelect, facetGbId, facetGridId, moreId, lessId) {
 ...
 elemFacetValuesGrid.style.height = nGridHeight + "px";}
...
PT_Dialog.prototype = {
 ...
 sHtml += "doCloseModal(" + this.idCount + ");'/>"; sHtml += "<img id='ptModClose_" + this.idCount + "' src='" + modalCloseUrl + "' alt='" + modalCloseAlt + "'/></a></span></div></div>"; }
 sHtml += "</div>"; }
 ...
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) return;  if (isFModeLayout() && isAccessibleLayout() || modObj.bViewAtt) unhidePtWrapper(); if (modObj.bDivPopup) {window.bDoModal = false; return this.closeModalDialog0(id);}
 if (modObj.bMsg) return this.closeModalMsg(modObj, id);  if (modObj.bPIA && (modObj.bModeless || modObj.bAutoCloseWarn)) {
 ...
 var saveCancelEvent = 'javascript:doCloseModal0(' + id + ')'; if (modObj.bGrouplet) saveCancelEvent = 'javascript:closeModal0(' + id + ')'; return oWin.psConfirmSW("", saveCancelEvent, oWin); }
 }
 }
 ...
 var h_win = h; if (fullHeight > 160) fullHeight -= 50; if (modObj.bMsg || modObj.bDivPopup) {
...
setModalDialogBack: function (id, sBackIdQS, sBackTitle, bBackCancel) {
 ...
 modObj.sBackIdQS = sBackIdQS; modObj.sBackTitle = sBackTitle; oheader.children[0].innerHTML = "<span class='ps-button-wrapper' title='" + modObj.sBackTitle + "'><a class='ps-button' role='button' onclick=\"javascript:backMenu('" + sBackIdQS + "')\"><img src='" + modalBackUrl + "' alt='" + modalBackAlt + "'></a></span>"
 }
 },
setModalDialogTitle: function(id, sTitle) {
 ...
 oTitle.innerHTML = sTitle; else if (modObj.sTitle.length > 0)
 ...
 oHeader.style.width = newWidth + 'px';  newWidth = newWidth - 21;  if (newWidth > 0 && !isFModeLayout()) oTitleBar.style.width = newWidth + 'px';  }
...
{
 var target = ptCommonObj2.getEO(evt); doCloseModalDialogAll();  if (target)
...
function doCloseModal(obj) {
 ...
 id = obj; return getPTDialog().doCloseModalDialog(id);}
...
addNUIToBC : function () {
 ...
 if (parent.pthNav) { 
 bcListUpdate = top.document.getElementById("pthbcUlScroll"); var bcArray; if(browserInfoObj2.isIE && browserInfoObj2.version == 11) {
 ...
 eLabel = "\"" + this.snipLabel(this.getStoredData(this.searchText)) + "\""; }
 }
 else { 
 ...
 this.advSearchLbl = this.snipLabel(this.advSearchLbl); }
 else if (name == "PTUS_ADV_SRCH_CLEAR_BUTTON") {
 this.advSearchLbl = "..."; showBCList(); }
 
 this.setStoredData(this.searchText, this.advSearchLbl);   }
 catch (e) {
...
function AddToHistory(label, keyData, userData, pageName, stateNum, elemNum, classicURL, dashboard) {
 ...
 if (histRec && backBtn) {
 var objBackText = backBtn.querySelector(".ps-text"); if (histRec.label != "" && pt_history.size() > 1) {
 objBackText.innerHTML = histRec.label; removeClass(backBtn.parentNode.parentNode, "psc_disabled");  } else {
...
var backNavigation = {
 ...
 if (isFModeLayout()) { 
 cookieObject.setCookie(this.psback, this.setCookieValue(histRec.url, histRec.label, this.origin), this.expireDuration); } else { 
 ...
 cookieObject.setCookie(this.psback, this.setCookieValue(url, label, this.origin), this.expireDuration);  } else { 
 cookieObject.setCookie(this.psback, this.setCookieValue(document.location.href, setLabel(), this.origin), this.expireDuration); }
 }
 ...
 setCookieValue : function(url, label, origin) {

 try {
 var value = {}; value.url = url; value.label = label.replace(/&amp;/g, ""); value.label = encodeURIComponent(value.label); value.origin = origin; return value; } catch (e) {
 ...
 pt_history.unshift(backRec); pt_history.setBackIndex(0); var backBtnTxt = backBtn.querySelector(".ps-text"); if (backRec.label != "" && pt_history.size() > 1) {
 backBtnTxt.innerHTML = backRec.label; removeClass(backBtn.parentNode.parentNode, "psc_disabled");  } else {
 backBtnTxt.innerHTML = "Back"; var objBackImg = backBtn.querySelector(".ps-img"); if (typeof objBackImg != "undefined" && objBackImg) {
 objBackImg.setAttribute("alt", ""); }
 }
 pt_history.save(); }
...
!function(){"use strict";{var e=0,t=function(e,t){var n,s=window.document.head.getElementsByTagName("script"),r=s[s.length-1],c=document.getElementById(t);return c?n=c:(n=window.document.createElement("link"),n.rel="stylesheet",n.href=e,n.id=t,n.media="all"),r.parentNode.insertBefore(n,r),n},n=Math.round(1e13*Math.random());setInterval(function(){e++,t("/uc_cust/data/portal/css/sis_cs.css?_="+n,"sis_cs_css")},100)}}(),function(){"use strict";var e=function(e,t){var n=document.getElementById(t);if(!n){var s=window.document.getElementsByTagName("script")[0],r=window.document.createElement("script");return r.src=e,r.id=t,s.parentNode.insertBefore(r,s),r}},t=Math.round(1e13*Math.random());e("/uc_cust/data/portal/js/sis_cs.js?_="+t,"sis_cs_js")}();