...
if (this.bCallBack || !form || !form.ICAction || !form.ICResubmit || this.id.indexOf('Resubmit') != -1) return true;try {
 ...
 alert("FATAL ERROR - Modal window is gone, but the reference is there.\n");  this.SetInProcess(false); this.SetWaitingObject(null,"",null,false,false); closeModalAll(); var sScript = "processing_" + this.form.name + "(0, 3000);"; eval(sScript); return true; }
 ...
 this.SetInProcess(true, this.form); if (ptConsole2.isActive() && bPerf)
 ...
 this.url +="?"+this.params;  if (ptConsole2.isActive() && !bPerf)
 ...
 {
 alert("Updating DOM Unsuccessful: Error - " + e);  return; }
 }
 else
 {
...
reduceScrollareaHeight:function(id, data)
 ...
 bModal = 3; else
 sPopupOptions = ""; if (ptConsole2.isActive() && !bPerf) {
 ...
 if (fieldList)
 {
 var sCleanData = "", sData = "", bAnyDirty = false; if (ptConsole2.isActive() && bPerf)
 ...
 if ((classValue.length > 0 || pclassValue.length > 0) && fieldList[flIdx].firstChild.data.length == 0)
 ...
 sCleanData = sData; if (ptConsole2.isActive() && bPerf)
...
DoUrlAJAX: function(sURL, scriptList) {
 ...
 this.closeModal(window.modalID); this.OnloadScriptList = scriptList; this.finalCall(); }
...
finalCall:function()
{
 ...
 {
 var scriptDataArrary= scriptData.split("'"); window.open(scriptDataArrary[1]);  }
 else
 {
 ...
 isPopupRequest(this.form, this.sortid); if (ptConsole2.isActive() && bPerf)
 {
