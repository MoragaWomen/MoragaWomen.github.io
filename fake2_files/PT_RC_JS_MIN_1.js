...
else if(key == 38) 
{
 ...
 this.rcMenu = new RCMenu(); rcMenuStr = "<div id='ptrcServiceTab'><ul role='presentation'>"; var tc = window.frames['TargetContent']; var isTangSS = false; if(typeof(tc) != "undefined" && tc != null)
...
onServiceByScroll:function (serviceName,scrollFieldName,bManual,bRefresh,nOpenMode,nInstID,sRecFldName,sPageFieldValues) {
 ...
 {
 if (rcMenuSpan[i].className == "ptrcMenuItem") {
 hrefURL = this.getHrefURL(rcMenuSpan[i].innerHTML); var URLServiceName = this.getServiceNameFromURL(hrefURL); var servInstID = this.getServiceInstID(hrefURL); if (szPageRCFFields != "")
 {
 var servLst = szPageRCFFields.split("~"); for (var j=0; j < servLst.length; j++)
 {
...
szPageRCFFields = sPageRCFFields;}
}
